...
namespace ASPNET.StarterKit.BusinessLogicLayer {
  public class TimeEntry {
    ...
    public static Decimal totalhours;

    /*** CONSTRUCTOR ***/
    public TimeEntry(string creatorUserName,int ProjectId, int categoryId, decimal duration, DateTime reportedDate, string userName)
    
        : this(creatorUserName,ProjectId, categoryId, DefaultValues.GetDateTimeMinValue(), string.Empty, duration, DefaultValues.GetTimeEntryIdMinValue(), reportedDate, userName)
    {
    }

    public TimeEntry(string creatorUserName,int ProjectId, int categoryId, DateTime dateCreated, string description, decimal duration, int id, DateTime reportedDate, string userName)
    {
    ...
    public TimeEntry(string creatorUserName, int ProjectId, int categoryId, DateTime dateCreated, string description, decimal duration, int id, DateTime reportedDate, string userName, string ProjectName)
    {
        if (String.IsNullOrEmpty(creatorUserName))
            throw (new NullReferenceException("creatorUserName"));
        ...
        if (String.IsNullOrEmpty(ProjectName))
            throw (new NullReferenceException("ProjectName"));
    ...
    public TimeEntry(string creatorUserName, int categoryId,string ProjectName, DateTime dateCreated, string description, decimal duration, int id, DateTime reportedDate, string userName, DateTime reportedOnDate)
    {
        if (String.IsNullOrEmpty(creatorUserName))
            throw (new NullReferenceException("creatorUserName"));
        ...
        if (reportedDate <= DefaultValues.GetDateTimeMinValue())
            throw (new ArgumentOutOfRangeException("reportedDate"));
        if (reportedOnDate <= DefaultValues.GetDateTimeMinValue())
            throw (new ArgumentOutOfRangeException("reportedOnDate"));
        if (String.IsNullOrEmpty(ProjectName))
            throw (new NullReferenceException("ProjectName")); ;
    ...
    public static List<TimeEntry> GetTimeEntriesByUserNameProjectIdAndDates(string userName, string ProjectId, DateTime startingDate, DateTime endDate)
    {
        if (String.IsNullOrEmpty(userName))
            return (new List<TimeEntry>());
        //if (ProjectId != null)
        ...
        //}
        DataAccess DALLayer = DataAccessHelper.GetDataAccess();
        return (DALLayer.GetTimeEntriesByUserNameProjectIdAndDates(userName, ProjectId, startingDate, endDate));
    }

    public static List<TimeEntry> GetTimeEntriesByUserNameProjectIdAndMonthAndYear(string userName, string ProjectId, int Month, int Year)
    {
        if (String.IsNullOrEmpty(userName))
            return (new List<TimeEntry>());
        ...
        DataAccess DALLayer = DataAccessHelper.GetDataAccess();
        ProjectId = ProjectId.Trim().Substring(0,ProjectId.Length-1);
        return (DALLayer.GetTimeEntriesByUserNameProjectIdAndMonthAndYear(userName, ProjectId, Month, Year));
    }

    public static bool UpdateTimeEntry(int Id, string Description, decimal Duration, DateTime ReportedDate, string UserName) {
